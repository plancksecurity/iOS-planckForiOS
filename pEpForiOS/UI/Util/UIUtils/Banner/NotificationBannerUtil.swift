...
class NotificationBannerUtil: NotificationBannerUtilProtocol {
    ...
    public static func show(errorMessage: String) {
        ...
        DispatchQueue.main.async {
...
#if EXT_SHARE
            guard currentlyShownViewController is ComposeViewController else {
            ...
            UIView.animate(withDuration: animateDuration) {
                ...
                if let vc = currentlyShownViewController as? ComposeViewController {
    ...
    public static func hide() {
        DispatchQueue.main.async {
            ...
            if let tv = currentlyShownViewController.view.subviews.filter({$0 is UITableView} ).first {
...
extension NotificationBannerUtil {

    public static func show(errorMessage: String, sharingExtensionComposeViewController: ComposeViewController) {
        DispatchQueue.main.async {
            guard let navigationBar = sharingExtensionComposeViewController.navigationController?.navigationBar else {
                // Navigation bar not found, nothing to do.
                return
            }
            ...
            // First, increase the distance between the tableView and the top of the view that is being displayed.
            UIView.animate(withDuration: animateDuration) {
                sharingExtensionComposeViewController.tableView.transform = CGAffineTransform(translationX: 0, y: bannerHeight)
                bannerHeightConstraint.constant = bannerHeight
                navigationBar.layoutIfNeeded()
            } completion: { finished in
                // Then, show the UI components.
    ...
    public static func hide(sharingExtensionComposeViewController: ComposeViewController) {
        DispatchQueue.main.async {
            if let tv = sharingExtensionComposeViewController.view.subviews.filter({$0 is UITableView} ).first {
                tv.transform = .identity
            }
