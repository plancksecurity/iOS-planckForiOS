...
public class VerifiableAccount: VerifiableAccountProtocol {
    ...
    public func save() throws {
        ...
        moc.performAndWait {
            ...
            // TODO: Reuse server!
            if let theServer = cdAccount.server(type: .imap) {
                delete(server: theServer, fromAccount: cdAccount)
            }

            // TODO: Reuse server!
            if let theServer = cdAccount.server(type: .smtp) {
                delete(server: theServer, fromAccount: cdAccount)
            }
    ...
    private func updateOrCreateOwnIdentity(context: NSManagedObjectContext,
        ...
        if let theAddress = address,
            let identity = CdIdentity.search(address: theAddress, context: context) {
            update(identity: identity, address: address, userName: userName)
            return identity
        } else {
        let cdId = CdIdentity(context: context)
